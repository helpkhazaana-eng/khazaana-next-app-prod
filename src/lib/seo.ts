...
const getSiteUrl = () => process.env.NEXT_PUBLIC_SITE_URL || 'https://khazaana.co.in';
...
export function generateMetaKeywords(category?: string): string {
  ...
  if (category) {
    ...
    if (catLower === 'biryani') categoryKeywords = FOOD_KEYWORDS.biryani;
    else if (catLower === 'chinese') categoryKeywords = FOOD_KEYWORDS.chinese;
    else if (catLower === 'cafe') categoryKeywords = FOOD_KEYWORDS.cafe;
    else categoryKeywords = FOOD_KEYWORDS.general;
  } else {
...
// Generate structured data for a specific restaurant
export function generateRestaurantSchema(restaurant: any) {
  return {
    '@context': 'https://schema.org',
    '@type': 'Restaurant',
    name: restaurant.name,
    image: `${getSiteUrl()}/images/restaurants/${restaurant.id}.jpg`, // Assuming image path pattern
    ...
    geo: {
      '@type': 'GeoCoordinates',
      latitude: 24.619968473610793, // Fallback if restaurant specific coords missing
      longitude: 88.0229246716395
    },
    openingHoursSpecification: {
      '@type': 'OpeningHoursSpecification',
    ...
    aggregateRating: {
      ...
      reviewCount: '50' // Placeholder/Estimate
    }
  };
}
