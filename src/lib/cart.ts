...
export function addToCart(
  ...
  if (!newCart.restaurantId || newCart.items.length === 0) {
  ...
  // Recalculate totals
  const updatedCart = calculateCartTotals(newCart, offers);
 ...
 */
export function removeFromCart(cart: Cart, itemName: string, offers: ExclusiveOffer[] = []): Cart {
  const newCart = {
    ...cart,
 ...
 */
export function updateQuantity(cart: Cart, itemName: string, quantity: number, offers: ExclusiveOffer[] = []): Cart {
  if (quantity <= 0) {
    return removeFromCart(cart, itemName, offers);
  }
 ...
 */
export function calculateCartTotals(cart: Cart, offers: ExclusiveOffer[] = []): Cart {
  const subtotal = cart.items.reduce(
    (sum, item) => {
      // Use discounted price for specific items (legacy logic)
      // TODO: Move this to offers logic too if possible, but keeping for now
  ...
  // Find applicable delivery offer
  const deliveryOffer = offers.find(o => 
    o.offerType === 'delivery' && 
    o.isActive && 
    o.restaurantId === cart.restaurantId &&
    (!o.minOrderValue || baseSubtotal >= o.minOrderValue)
  );

  if (!cart.items.length) {
    deliveryFee = 0;
  } else if (deliveryOffer) {
    // If offer exists and condition met, delivery is free (or discounted)
    deliveryFee = 0;
  } else {
    // Legacy hardcoded fallbacks
    if (cart.restaurantId === 'cupsncrumbs' && baseSubtotal >= 100) {
      deliveryFee = 0;
