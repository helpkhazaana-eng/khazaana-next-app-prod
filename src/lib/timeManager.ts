 ...
 * - Automatic cleanup on page hide (bfcache-friendly)
...
const cache = {
  ...
  countdown: '',
  lastISTHour: -1,
  overrideStatus: 'auto' as 'open' | 'closed' | 'auto',
};
...
export interface TimeData {
  ...
  overrideStatus: 'open' | 'closed' | 'auto';
}
...
function isRestaurantOpen(istHour: number, override: 'open' | 'closed' | 'auto' = 'auto'): boolean {
...
function generateCountdown(istTime: Date, istHour: number, override: 'open' | 'closed' | 'auto' = 'auto'): string {
  const isOpen = isRestaurantOpen(istHour, override);
  
  if (override === 'closed') return 'Temporarily Closed';
  if (override === 'open') return 'Open now';
...
function computeTimeData(override: 'open' | 'closed' | 'auto' = cache.overrideStatus): TimeData {
  ...
  return {
    ...
    istHour,
    istMinute,
    countdown: generateCountdown(istTime, istHour, override),
    minutesUntilOpen,
    minutesUntilClose,
    isClosingSoon: isOpen && minutesUntilClose <= 30 && override === 'auto',
    isOpeningSoon: !isOpen && minutesUntilOpen <= 30 && override === 'auto',
...
// Auto-cleanup using pagehide (bfcache-friendly alternative to unload/beforeunload)
// Using unload/beforeunload blocks bfcache - pagehide is the modern alternative
if (typeof window !== 'undefined') {
  window.addEventListener('pagehide', cleanupTimeManager);
}
