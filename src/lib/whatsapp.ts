...
Thank you for ordering with Khazaana!`;
  ...
  const encodedMessage = encodeURIComponent(message);
  // Use provided system number or fallback to hardcoded default
  const numberToUse = systemPhoneNumber || KHAZAANA_WHATSAPP;
  const cleanPhone = numberToUse.replace(/\D/g, '');
